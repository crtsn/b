...
pub struct Assembler {
    ...
    pub string_links: Array<(usize, usize, usize)>,
...
pub unsafe fn load_arg(arg: Arg, loc: Loc, out: *mut String_Builder, asm: *mut Assembler, p: *const Program) -> Option<()> {
    match arg {
        ...
        Arg::String(string_number) => {
            ...
            da_append(&mut (*asm).string_links, (low_reloc, high_reloc, string_number));
        },
            ...
            match *global.values.items.add(j) {
                ImmediateValue::IntLiteral(int_literal, radix) => {
                    ...
                    if let Ok(v) = parse_int_literal_to_u16(int_literal, radix) {
                        ...
                        diagf!(*global.value_locs.items.add(j), c!("ERROR: mos6502: constant %s%s out of range for 16 bits\n"), prefix, int_literal);
                        value = bump_error_count((*p).error_count).map(|()| 0)?;
                    }
...
pub unsafe fn generate_strings(out: *mut String_Builder, file_data_section_start: u16, asm: *mut Assembler, strings: *const [String]) {
    for i in 0..(*asm).string_links.count {
        let string_link = *(*asm).string_links.items.add(i);
        let string = (*strings)[string_link.2];
        if (*out).count%2 == 1 {
            write_byte(out, 0);
        }
        let Relocation{kind: RelocationKind::DataOffset{ off: ref mut low_reloc, .. }, ..} = &mut *(*asm).relocs.items.add(string_link.0) else {
            unreachable!();
        };
        let Relocation{kind: RelocationKind::DataOffset{ off: ref mut high_reloc, .. }, ..} = &mut *(*asm).relocs.items.add(string_link.1) else {
            unreachable!();
        };
        ...
        *high_reloc = (*out).count as u16 - file_data_section_start;
        for word in 0..string.count/2 {
            let low = *string.string.add(word * 2 + 1) as u8;
            write_byte(out, low);
            let high = *string.string.add(word * 2) as u8;
            write_byte(out, high);
        }
        if string.count%2 == 1 {
...
pub unsafe fn generate_program(
    ...
    generate_strings(out, file_data_section_start, &mut asm, da_slice((*p).strings));
